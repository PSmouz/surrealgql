...
macro_rules! parse_field {
    (
        ...
        trace!("field {}, kind {:?}, type {:?}, type_name {:?}", fd_name, kind.clone(), fd_ty.clone
        (), fd_ty
        .type_name());

        if kind_non_optional == Kind::Object {
            // object map used to add fields step by step to the objects
            let obj_q = Object::new(fd_ty.type_name()).description(description!($fd));
            let obj_c = InputObject::new(format!("Create{}Input", fd_ty.type_name()))
                .description(description!($fd));
            let obj_u = InputObject::new(format!("Update{}Input", fd_ty.type_name()))
                .description(description!($fd));
            trace!("Adding nested object for field {}: {:?}", fd_name, fd_ty.type_name());
            trace!("Objects: {:?}, {:?}, {:?}", obj_q.type_name(), obj_c.type_name(), obj_u.type_name());
        ...
        // `NonNull(Named("Tablename"))` for records, we cannot use this type for mutations.
        // In input fields we want the user to provide the record ID, not a record object.
        if kind.is_record() {
        ...
        let fd_c = InputValue::new(
            ...
            if kind_non_optional == Kind::Object {
                ...
                fd_ty.clone() // Only the originally optional types are optional
            }
        )
        .description(description!($fd));
        ...
        let fd_u = InputValue::new(
            &fd_name_gql,
            if kind_non_optional == Kind::Object {
                unwrap_type(fd_ty.rename(format!("Update{}Input", fd_ty.type_name())))
            } else {
                unwrap_type(fd_ty.clone()) // Make every type optional
            }
        )
        .description(description!($fd));
